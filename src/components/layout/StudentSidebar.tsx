...
import {
  ...
  User, LogOut, ChevronRight, ClipboardList, FolderOpen, ChevronDown, Brain, BarChart2, X,
} from 'lucide-react'
...
export default function StudentSidebar({ collapsed, onToggle, mobileOpen = false, onMobileClose }: SidebarProps) {
          ...
          {NAV_ITEMS.map((item) => {
              ...
              return (
                <div key={item.label}>
                  ...
                  <button
                    ...
                    onClick={() => !collapsedView && toggleGroup(item.label)}
                  ...
                  <AnimatePresence initial={false}>
                    {(isGroupOpen || collapsedView) && (
                      <motion.div
                        key="children"
                        initial={collapsedView ? false : { height: 0, opacity: 0 }}
                      ...
                      >
                        {item.children.map(child => (
                          <NavLink
                            ...
                            className={({ isActive }) =>
                              `sidebar-link sub-link ${isActive ? 'active' : ''} ${collapsedView ? 'collapsed' : ''}`
                            }
            ...
            return (
              <NavLink
                ...
                onClick={handleNavClick}
                className={({ isActive }) =>
                  `sidebar-link ${isActive ? 'active' : ''} ${collapsedView ? 'collapsed' : ''}`
                }
              >
                <div className="sidebar-link-icon">
                  <Icon size={18} strokeWidth={1.8} />
